...
int main() {
  ...
  for (int i = a; i <= b; ++i) {
    std::cout << i << " " << apply_function_verbose(square, i) << " "
...
int apply_function_verbose(int (*f)(int x), int x) { return f(x); }

int apply_function_concise(int f(int x), int x) { return f(x); }

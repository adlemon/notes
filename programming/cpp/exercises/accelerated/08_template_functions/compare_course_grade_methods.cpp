...
double course_grade(const StudentCourseGrades &student,
                    HomeworkAverageFunction get_homework_average);

template <class HomeworkAverageFunction>
double median_course_grade(const std::vector<StudentCourseGrades> &students,
                           HomeworkAverageFunction get_homework_average);

template <class HomeworkAverageFunction>
void write_analysis(
    std::ostream &out, const std::string &analysis_method_name,
    HomeworkAverageFunction get_homework_average,
...
int main() {
  ...
  write_analysis(std::cout, "median of homework scores", median,
                 students_did_all_hw, students_missing_hw);

  write_analysis(std::cout, "mean of homework scores", mean,
                 students_did_all_hw, students_missing_hw);

  write_analysis(std::cout, "median of nonzero homework scores", median_nonzero,
                 students_did_all_hw, students_missing_hw);
}
...
double median_nonzero(const std::vector<double> &v) {
  ...
  std::remove_copy(v.begin(), v.end(), std::back_inserter(v_nonzero), 0);
...
void write_analysis(
    ...
    const std::vector<StudentCourseGrades> &students_did_all_hw,
    const std::vector<StudentCourseGrades> &students_missing_hw) {
  out << analysis_method_name << ": median(did all hw) = "

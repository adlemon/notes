...
class LinkedList(object):
    ...
    def __str__(self):
        return ('{} -> {}'.format(self.data, self.next)
                if self.next else str(self.data))
...
def make_linked_list_from_list(lst):
    return (LinkedList(lst[0], make_linked_list_from_list(lst[1:]))
            if lst else None)


def main():
    tests = (
        (make_linked_list_from_list([1]), make_linked_list_from_list([1])),
        (make_linked_list_from_list([1, 2]), make_linked_list_from_list([2,
                                                                         1])),
        (make_linked_list_from_list([1, 2,
                                     3]), make_linked_list_from_list([3, 2,
                                                                      1])),
